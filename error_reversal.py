...
def load_notes_from_file(filename):
    ...
    try:
        with open(filename, 'r', encoding='utf-8') as file:
            ...
            if notes is None:
                return []  # Возвращаем пустой список, если файл пуст или не содержит данных
            return notes
...
def main():
    ...
    filename = "notes.yaml"
    notes = load_notes_from_file(filename)  # Загрузка заметок из файла при старте
    if notes is None:
        notes = []
    note_id_counter = len(notes) + 1  # Восстанавливаем счетчик
    ...
    while True:
        ...
        if choice == '1':
            ...
            save_notes_to_file(notes, filename)  # Сохранение заметок перед выходом
