...
def check_deadline(): # Функция для проверки дедлайна
    ...
    issue_date_obj = datetime.datetime.strptime(f'{issue_date}-2023', '%d.%m-%Y').date()

    if today > issue_date_obj:
        print("Внимание! Срок выполнения заметки истек!\n")
    else:
        days_left = (issue_date_obj - today).days
        print(f"Осталось {days_left} дней до окончания срока выполнения заметки.\n")
